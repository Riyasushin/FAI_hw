...
def train():
    ...
    for epoch in range(flags["n_epoch"]):
        ...
        for i, (real_images, _) in enumerate(train_loader):
            ...
            y_real = torch.ones(batch_size, device=device)
            y_fake = torch.zeros(batch_size, device=device)
            ...
            d_loss_real = criterion(output_real, y_real)

            z_fake = torch.randn(batch_size, flags["z_dim"], device=device)
            fake_img = G(z_fake)
            output_fake = D(fake_img.detach()) # !!detach
            d_loss_fake = criterion(output_fake, y_fake)

            d_loss = (d_loss_fake * flags["d_fake_loss_weight"] + d_loss_real * flags["d_real_loss_weight"]) / (flags["d_fake_loss_weight"] + flags["d_real_loss_weight"]) # TODO 改变权重？？？
            ...
            z_fake = torch.randn(batch_size, flags["z_dim"], device=device)
            for _ in range(flags["g_train_iter_per_epoch"]):
